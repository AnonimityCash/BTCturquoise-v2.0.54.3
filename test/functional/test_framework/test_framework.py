#!/usr/bin/env python3
# Copyright (c) 2014-2016 The BTCturquoise Core developers
...
class BTCturquoiseTestFramework():
    """Base class for a btcturquoise test script.

    Individual btcturquoise test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave btcturquoiseds and test.* datadir on exit or error")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing btcturquoised/btcturquoise-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use btcturquoise-cli instead of RPC for all commands")
        parser.add_option("--btcturquoised-arg", dest="dashd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all btcturquoised instances")
        ...
        self.options.btcturquoised = os.getenv("BTCTURQUOISED", default=config["environment"]["BUILDDIR"] + '/src/btcturquoised' + config["environment"]["EXEEXT"])
        self.options.btcturquoisecli = os.getenv("BTCTURQUOISECLI", default=config["environment"]["BUILDDIR"] + '/src/btcturquoise-cli' + config["environment"]["EXEEXT"])
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: btcturquoiseds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
        ...
        if binary is None:
            binary = [self.options.btcturquoised] * num_nodes
        assert_equal(len(extra_confs), num_nodes)
        assert_equal(len(extra_args), num_nodes)
        assert_equal(len(binary), num_nodes)
        old_num_nodes = len(self.nodes)
        for i in range(num_nodes):
            self.nodes.append(TestNode(old_num_nodes + i, get_datadir_path(self.options.tmpdir, old_num_nodes + i), self.extra_args_from_options, chain=self.chain, rpchost=rpchost, timewait=timewait, btcturquoised=binary[i], btcturquoise_cli=self.options.btcturquoisecli, stderr=stderr, mocktime=self.mocktime, coverage_dir=self.options.coveragedir, extra_conf=extra_confs[i], extra_args=extra_args[i], use_cli=self.options.usecli))

    def start_node(self, i, *args, **kwargs):
        """Start a btcturquoised"""
    ...
    def stop_node(self, i, wait=0):
        """Stop a btcturquoised test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple btcturquoised test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as btcturquoised's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("BTCturquoiseRPC")
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run btcturquoiseds:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i, self.chain)
                args = [self.options.btcturquoised, "-datadir=" + datadir, "-mocktime="+str(GENESISTIME)]
                if i > 0:
                    args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
                if extra_args is not None:
                    args.extend(extra_args)
                self.nodes.append(TestNode(i, get_datadir_path(self.options.cachedir, i), chain=self.chain, extra_conf=["bind=127.0.0.1"], extra_args=[],extra_args_from_options=self.extra_args_from_options, rpchost=None, timewait=None, btcturquoised=self.options.btcturquoised, btcturquoise_cli=self.options.btcturquoisecli, stderr=stderr, mocktime=self.mocktime, coverage_dir=None))
...
class BTCturquoiseTestFramework(BTCturquoiseTestFramework):
    def set_btcturquoise_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
def skip_if_no_btcturquoised_zmq(test_instance):
    """Skip the running test if btcturquoised has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("btcturquoised has not been built with zmq enabled.")
