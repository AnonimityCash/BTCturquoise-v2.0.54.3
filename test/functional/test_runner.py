...
`test/functional/test_framework/test_framework.py:BTCturquoiseTestFramework.main`.
...
EXTENDED_SCRIPTS = [
    ...
    'mining_getblocktemplate_longpoll.py',  # FIXME: "socket.error: [Errno 54] Connection reset by peer" on my Mac, same as  https://github.com/btcturquoise/btcturquoise/issues/6651
    'p2p_timeouts.py',
    # vv Tests less than 60s vv
    # vv Tests less than 30s vv
...
def main():
    ...
    # args to be passed on always start with two btcturquoisees; tests are the remaining unknown args
    ...
    tmpdir = "%s/btcturquoise_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_btcturquoised = config["components"].getboolean("ENABLE_BTCTURQUOISED")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/btcturquoise/btcturquoise/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/btcturquoise/btcturquoise/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_btcturquoised):
        print("No functional tests to run. Wallet, utils, and btcturquoised must all be enabled")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, failfast=False, runs_ci, combined_logs_len=0):
    ...
    # Warn if btcturquoised is already running (unix only)
    try:
        pidof_output = subprocess.check_output(["pidof", "btcturquoised"])
        if not (pidof_output is None or pidof_output == b''):
            print("%sWARNING!%s There is already a btcturquoised process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `btcturquoise-cli help` (`rpc_interface.txt`).
