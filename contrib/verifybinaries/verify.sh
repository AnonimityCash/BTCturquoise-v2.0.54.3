...
###   btcturquoisecore.org and btcturquoise.org and compares them.
...
WORKINGDIR="/tmp/btcturquoise_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://btcturquoisecore.org"
HOST2="https://btcturquoise.org"
BASEDIR="/bin/"
VERSIONPREFIX="btcturquoise-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'btcturquoise-',
...
if [ $? -ne 0 ]; then
   echo "btcturquoise.org failed to provide signature file, but btcturquoisecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "btcturquoise.org and btcturquoisecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BTCturquoise Core binary release signing key installed?"
   fi
