...
public:
    ...
    // BTCturquoise Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between btcturquoised, and btcturquoise-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("btcturquoise-shutoff");
    ...
    // if (g_txindex) g_txindex->Stop(); //TODO watch out when backporting btcturquoise#13033 (don't accidently put the reset here, as we've already backported btcturquoise#13894)
    ...
    peerLogic.reset();
    g_connman.reset();
    //g_txindex.reset(); //TODO watch out when backporting btcturquoise#13033 (re-enable this, was backported via btcturquoise#13894)
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", BTCTURQUOISE_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BTCTURQUOISE_PID_FILENAME), false, OptionsCategory::OPTIONS);
#endif
    ...
    gArgs.AddArg("-sporkaddr=<btcturquoiseaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by BTCturquoise Platform, to the specified username.", false, OptionsCategory::SMARTNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/btcturquoiseverse/btcturquoise>";
    const std::string URL_WEBSITE = "<https://btcturquoiseverse.com>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("btcturquoise-loadblk");
...
/** Sanity checks
 *  Ensure that BTCturquoise Core is running in a usable environment with all
...
{
    // Make sure only a single BTCturquoise Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BTCTURQUOISE_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if BTCturquoise Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if BTCturquoise Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule BTCturquoise-specific tasks
