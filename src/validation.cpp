...
#if defined(NDEBUG)
# error "BTCturquoise Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in BTCturquoise
...
/*
NOTE:   unlike btcturquoise we are using PREVIOUS block height here,
        might be a good idea to change this to use prev bits
        but current height to avoid confusion.
*/
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	double nSubsidy = 6000;      // (declaring the reward variable and its original/default amount)
    ...
    if ((nPrevHeight > 103680) && (nPrevHeight < 207630)) {
        nSubsidy = 5700; 
...
CAmount GetSmartnodePayment(int nHeight, CAmount blockValue, CAmount specialTxFees)
{
    ...
    if (mnCount >= 8 || Params().NetworkIDString().compare("test") == 0) {
...
void ThreadScriptCheck() {
    RenameThread("BTCturquoise-scriptch");
...
static int64_t nTimeBTCturquoiseSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    /// BTCturquoise: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(BTCturquoise): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // BTCturquoise : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(BTCturquoise): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(BTCturquoise): spork is off, skipping transaction locking checks\n");
    }
    ...
    // BTCturquoise : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(BTCturquoise): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(BTCturquoise): couldn't find smartnode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeBTCturquoiseSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - BTCturquoise specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeBTCturquoiseSpecific * MICRO, nTimeBTCturquoiseSpecific * MILLI / nBlocksTotal);
