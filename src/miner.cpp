...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // NOTE: unlike in btcturquoise, we need to pass PREVIOUS block height here
...
void static BTCturquoiseMiner(const CChainParams& chainparams)
{
    LogPrintf("BTCturquoiseMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btcturquoise-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("BTCturquoiseMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BTCturquoiseMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("BTCturquoiseMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BTCturquoiseMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BTCturquoiseMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateBTCturquoises(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&BTCturquoiseMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
