...
const char * const BTCTURQUOISE_CONF_FILENAME = "btcturquoise.conf";
const char * const BTCTURQUOISE_PID_FILENAME = "btcturquoised.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "btcturquoised -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTCturquoiseCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTCturquoiseCore
    // Mac: ~/Library/Application Support/BTCturquoiseCore
    // Unix: ~/.btcturquoisecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTCturquoiseCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BTCturquoiseCore";
#else
    // Unix
    return pathRet / ".btcturquoisecore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty btcturquoise.conf if it does not excist
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BTCTURQUOISE_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BTCturquoise Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The BTCturquoise Core developers";
    }
    ...
    // Check for untranslated substitution to make sure BTCturquoise Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BTCturquoise Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Dash Core developers";
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The BTCturquoise Core developers";
    }
