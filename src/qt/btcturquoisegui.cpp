...
const std::string BTCturquoiseGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
BTCturquoiseGUI::BTCturquoiseGUI(interfaces::Node& node, const NetworkStyle* networkStyle, QWidget* parent) :
    QMainWindow(parent),
    enableWallet(false),
    m_node(node),
    clientModel(0),
...
BTCturquoiseGUI::~BTCturquoiseGUI()
{
...
void BTCturquoiseGUI::startSpinner()
{
...
void BTCturquoiseGUI::stopSpinner()
{
    if (timerSpinner == nullptr) {
        return;
    }
...
void BTCturquoiseGUI::startConnectingAnimation()
{
    static int nStep{-1};
...
void BTCturquoiseGUI::stopConnectingAnimation()
{
    if (timerConnecting == nullptr) {
        return;
    }
...
void BTCturquoiseGUI::createActions()
{
    sendCoinsMenuAction = new QAction(tr("&Send"), this);
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a BTCturquoise address"));
    sendCoinsMenuAction->setToolTip(sendCoinsMenuAction->statusTip());

    coinJoinCoinsMenuAction = new QAction("&CoinJoin", this);
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a BTCturquoise address").arg("CoinJoin"));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and btcturquoise: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about BTCturquoise Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BTCturquoise addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BTCturquoise addresses"));
    ...
    openAction->setStatusTip(tr("Open a btcturquoise: URI or payment request"));

    showHelpMessageAction = new QAction(QIcon(":/icons/configure"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BTCturquoise command-line options").arg(tr(PACKAGE_NAME)));
...
void BTCturquoiseGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void BTCturquoiseGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if(walletFrame)
    {
...
void BTCturquoiseGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool BTCturquoiseGUI::addWallet(WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BTCturquoiseGUI::removeWallet(WalletModel* walletModel)
{
...
bool BTCturquoiseGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BTCturquoiseGUI::setWalletActionsEnabled(bool enabled)
{
#ifdef ENABLE_WALLET
...
void BTCturquoiseGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void BTCturquoiseGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void BTCturquoiseGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void BTCturquoiseGUI::macosDockIconActivated()
{
...
void BTCturquoiseGUI::optionsClicked()
{
...
void BTCturquoiseGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BTCturquoiseGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
}

void BTCturquoiseGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void BTCturquoiseGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BTCturquoiseGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void BTCturquoiseGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void BTCturquoiseGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void BTCturquoiseGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void BTCturquoiseGUI::showBackups()
{
    GUIUtil::showBackups();
}

void BTCturquoiseGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void BTCturquoiseGUI::showCoinJoinHelpClicked()
{
    if(!clientModel)
        return;
...
#ifdef ENABLE_WALLET
void BTCturquoiseGUI::openClicked()
{
...
void BTCturquoiseGUI::highlightTabButton(QAbstractButton *button, bool checked)
{
    GUIUtil::setFont({button}, checked ? GUIUtil::FontWeight::Bold : GUIUtil::FontWeight::Normal, 16);
    GUIUtil::updateFonts();
}

void BTCturquoiseGUI::gotoOverviewPage()
{
    overviewButton->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BTCturquoiseGUI::gotoHistoryPage()
{
    historyButton->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BTCturquoiseGUI::gotoSmartnodePage()
{
    QSettings settings;
...
void BTCturquoiseGUI::gotoReceiveCoinsPage()
{
    receiveCoinsButton->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BTCturquoiseGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsButton->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BTCturquoiseGUI::gotoCoinJoinCoinsPage(QString addr)
{
    coinJoinCoinsButton->setChecked(true);
    if (walletFrame) walletFrame->gotoCoinJoinCoinsPage(addr);
}

void BTCturquoiseGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BTCturquoiseGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BTCturquoiseGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BTCturquoise network", "", count));
    } else {
...
void BTCturquoiseGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BTCturquoiseGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void BTCturquoiseGUI::updateHeadersSyncProgressLabel()
{
...
void BTCturquoiseGUI::updateProgressBarVisibility()
{
    if (clientModel == nullptr) {
        return;
    }
...
void BTCturquoiseGUI::updateCoinJoinVisibility()
{
#ifdef ENABLE_WALLET
...
void BTCturquoiseGUI::updateWidth()
{
    if (walletFrame == nullptr) {
        return;
    }
    ...
    // Use nButtonsVisible + 1 <- for the btcturquoise logo
    int nWidth = std::max<int>(980, (nWidthWidestButton + 30) * (nButtonsVisible + 1));
    setMinimumWidth(nWidth);
    resize(nWidth, height());
}

void BTCturquoiseGUI::setNumBlocks(int count, const QDateTime& blockDate, const QString& blockHash, double nVerificationProgress, bool header)
{
#ifdef Q_OS_MAC
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in btcturquoise
        {
...
void BTCturquoiseGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void BTCturquoiseGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BTCturquoise Core"); // default title
    ...
    // Append title to "BTCturquoise Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void BTCturquoiseGUI::changeEvent(QEvent *e)
{
...
void BTCturquoiseGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BTCturquoiseGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BTCturquoiseGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
...
void BTCturquoiseGUI::showIncomingTransactions()
{
    ...
    if (txs.size() >= 100) {
        ...
        QString msg;
        if (sentCount > 0) {
            msg += tr("Sent Amount: %1\n").arg(BTCturquoiseUnits::formatWithUnit(unit, sentAmount, true));
        }
        if (receivedCount > 0) {
            msg += tr("Received Amount: %1\n").arg(BTCturquoiseUnits::formatWithUnit(unit, receivedAmount, true));
        }
        ...
        for (auto& itx : txs) {
            ...
            QString msg = tr("Date: %1\n").arg(itx.date) +
                          tr("Amount: %1\n").arg(BTCturquoiseUnits::formatWithUnit(itx.unit, itx.amount, true));
...
void BTCturquoiseGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BTCturquoiseGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BTCturquoiseGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BTCturquoiseGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BTCturquoiseGUI::setHDStatus(int hdEnabled)
{
    if (hdEnabled) {
...
void BTCturquoiseGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BTCturquoiseGUI::updateWalletStatus()
{
    if (!walletFrame) {
        return;
    }
...
void BTCturquoiseGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BTCturquoiseGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BTCturquoiseGUI::detectShutdown()
{
    if (m_node.shutdownRequested())
    {
        if(rpcConsole)
...
void BTCturquoiseGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BTCturquoiseGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BTCturquoiseGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BTCturquoiseGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BTCturquoiseGUI::subscribeToCoreSignals()
{
...
void BTCturquoiseGUI::unsubscribeFromCoreSignals()
{
...
void BTCturquoiseGUI::toggleNetworkActive()
{
    m_node.setNetworkActive(!m_node.getNetworkActive());
}

/** Get restart command-line parameters and request restart */
void BTCturquoiseGUI::handleRestart(QStringList args)
{
    if (!m_node.shutdownRequested())
    ...
    menu(0)
{
    ...
    QList<BTCturquoiseUnits::Unit> units = BTCturquoiseUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(GUIUtil::getFontNormal());
    for (const BTCturquoiseUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(BTCturquoiseUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (BTCturquoiseUnits::Unit u : BTCturquoiseUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(BTCturquoiseUnits::name(u)), this);
...
{
    setText(BTCturquoiseUnits::name(newUnits));
}
