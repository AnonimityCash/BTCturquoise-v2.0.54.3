...
const int BTCTURQUOISE_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BTCTURQUOISE_IPC_PREFIX("btcturquoise:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/btcturquoise-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/btcturquoise-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/btcturquoise-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the btcturquoise: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BTCTURQUOISE_IPC_PREFIX, Qt::CaseInsensitive)) // btcturquoise: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseBTCturquoiseURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BTCTURQUOISE_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BTCTURQUOISE_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click btcturquoise: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start btcturquoise: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling btcturquoise: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in btcturquoise: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("btcturquoise://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'btcturquoise://' is not a valid URI. Use 'btcturquoise:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BTCTURQUOISE_IPC_PREFIX, Qt::CaseInsensitive)) // btcturquoise: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBTCturquoiseURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid BTCturquoise address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom btcturquoise addresses are not supported
        ...
        // BTCturquoise amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(BTCturquoiseUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
