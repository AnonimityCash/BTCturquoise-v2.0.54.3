...
public:
    explicit TxViewDelegate(QObject* parent = nullptr) :
        QAbstractItemDelegate(), unit(BTCturquoiseUnits::BTCQ)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        QString strAmount = BTCturquoiseUnits::floorWithUnit(unit, nAmount, true, BTCturquoiseUnits::separatorAlways);
        painter->setPen(colorForeground);
...
void OverviewPage::setBalance(const interfaces::WalletBalances& balances)
{
    ...
    m_balances = balances;
    ui->labelBalance->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelUnconfirmed->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.unconfirmed_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelImmature->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.immature_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelAnonymized->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.anonymized_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelTotal->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.balance + balances.unconfirmed_balance + balances.immature_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelWatchAvailable->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.watch_only_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelWatchPending->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.unconfirmed_watch_only_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelWatchImmature->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.immature_watch_only_balance, false, BTCturquoiseUnits::separatorAlways));
    ui->labelWatchTotal->setText(BTCturquoiseUnits::floorHtmlWithUnit(unit, balances.watch_only_balance + balances.unconfirmed_watch_only_balance + balances.immature_watch_only_balance, false, BTCturquoiseUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("BTCQ")
        updateDisplayUnit();
...
void OverviewPage::updateCoinJoinProgress()
{
    ...
    QString strCoinJoinAmount = BTCturquoiseUnits::formatHtmlWithUnit(nDisplayUnit, clientModel->coinJoinOptions().getAmount() * COIN, false, BTCturquoiseUnits::separatorAlways);
    ...
    if(m_balances.balance == 0)
    {
        ...
        strCoinJoinAmount = strCoinJoinAmount.remove(strCoinJoinAmount.indexOf("."), BTCturquoiseUnits::decimals(nDisplayUnit) + 1);
    ...
    if (nMaxToAnonymize >= clientModel->coinJoinOptions().getAmount() * COIN) {
        ...
        QString strMaxToAnonymize = BTCturquoiseUnits::formatHtmlWithUnit(nDisplayUnit, nMaxToAnonymize, false, BTCturquoiseUnits::separatorAlways);
        ...
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), BTCturquoiseUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = "<span style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_ERROR) + "'>" +
                QString(BTCturquoiseUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
...
void OverviewPage::toggleCoinJoin(){
    ...
    if (!walletModel->coinJoin().isMixing()) {
        ...
        if(m_balances.balance < nMinAmount) {
            QString strMinAmount(BTCturquoiseUnits::formatWithUnit(nDisplayUnit, nMinAmount));
            QMessageBox::warning(this, "CoinJoin",
