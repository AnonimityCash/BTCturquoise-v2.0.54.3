...
#include <qt/btcturquoiseaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BTCturquoise address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BTCturquoiseAddressEntryValidator(parent, fAllowURI));
    widget->setCheckValidator(new BTCturquoiseAddressCheckValidator(parent));
}
...
void setupAppearance(QWidget* parent, OptionsModel* model)
{
    if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
        ...
        dlg.setWindowIcon(QIcon(":icons/btcturquoise"));
...
bool parseBTCturquoiseURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no btcturquoise: URI
    if(!uri.isValid() || uri.scheme() != QString("btcturquoise"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BTCturquoiseUnits::parse(BTCturquoiseUnits::BTCQ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBTCturquoiseURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBTCturquoiseURI(uriInstance, out);
}

bool validateBTCturquoiseURI(const QString& uri)
{
    SendCoinsRecipient rcp;
    return parseBTCturquoiseURI(uri, &rcp);
}

QString formatBTCturquoiseURI(const SendCoinsRecipient &info)
{
    QString ret = QString("btcturquoise:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BTCturquoiseUnits::format(BTCturquoiseUnits::BTCQ, info.amount, false, BTCturquoiseUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BTCTURQUOISE_CONF_FILENAME));

    /* Open btcturquoise.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BTCturquoise Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BTCturquoise Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BTCturquoise Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "BTCturquoise Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "btcturquoisecore.desktop";
    return GetAutostartDir() / strprintf("btcturquoisecore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a btcturquoisecore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BTCturquoise Core\n";
        else
            optionFile << strprintf("Name=BTCturquoise Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the BTCturquoise Core app
...
bool GetStartOnSystemStartup()
{
    CFURLRef btcturquoiseAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (btcturquoiseAppUrl == nullptr) {
        return false;
    }

    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, btcturquoiseAppUrl);

    CFRelease(btcturquoiseAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BTCturquoise Core app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, btcturquoiseAppUrl, nullptr, nullptr);
    }
...
void loadStyleSheet(bool fForceUpdate)
{
    ...
    if (stylesheet == nullptr || fForceUpdate || fDebugCustomStyleSheets) {
        ...
        auto loadFiles = [&](const std::vector<QString>& vecFiles) -> bool {
            ...
            std::string platformName = gArgs.GetArg("-uiplatform", BTCturquoiseGUI::DEFAULT_UIPLATFORM);
...
void updateFonts()
{
    ...
    for (QWidget* w : qApp->allWidgets()) {
        std::vector<QString> vecIgnoreClasses{
            ...
            "QScrollBar", "QListView", "BTCturquoiseGUI", "WalletView", "WalletFrame"
        };
...
void disableMacFocusRect(const QWidget* w)
{
    ...
    for (const auto& c : w->findChildren<QWidget*>()) {
        if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
            c->setAttribute(Qt::WA_MacShowFocusRect, !btcturquoiseThemeActive());
...
void updateMacFocusRects()
{
    ...
    while (it != setRectsDisabled.end()) {
        if (allWidgets.contains(*it)) {
            (*it)->setAttribute(Qt::WA_MacShowFocusRect, !btcturquoiseThemeActive());
            ++it;
        } else {
