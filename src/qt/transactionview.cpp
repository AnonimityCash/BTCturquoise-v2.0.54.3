...
void TransactionView::changedAmount()
{
    ...
    // Replace "," by "." so BTCturquoiseUnits::parse will not fail for users entering "," as decimal separator
    QString newAmount = amountWidget->text();
    newAmount.replace(QString(","), QString("."));

    if (BTCturquoiseUnits::parse(model->getOptionsModel()->getDisplayUnit(), newAmount, &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(BTCturquoiseUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::showAddressQRCode()
{
    ...
    dialog->setInfo(tr("QR code"), "btcturquoise:"+strAddress, "", strAddress);
    dialog->show();
}
...
void TransactionView::computeSum()
{
    ...
    QString strAmount(BTCturquoiseUnits::formatWithUnit(nDisplayUnit, amount, true, BTCturquoiseUnits::separatorAlways));
