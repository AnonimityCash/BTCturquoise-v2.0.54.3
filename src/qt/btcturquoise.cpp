...
{
    return QCoreApplication::translate("btcturquoise-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. btcturquoise_de.qm (shortcut "de" needs to be defined in btcturquoise.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. btcturquoise_de_DE.qm (shortcut "de_DE" needs to be defined in btcturquoise.qrc)
...
/** Class encapsulating BTCturquoise Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class BTCturquoiseCore: public QObject
{
    Q_OBJECT
public:
    explicit BTCturquoiseCore(interfaces::Node& node);
...
/** Main BTCturquoise application object */
class BTCturquoiseApplication: public QApplication
{
    Q_OBJECT
public:
    explicit BTCturquoiseApplication(interfaces::Node& node, int &argc, char **argv);
    ~BTCturquoiseApplication();
    ...
    /// Get window identifier of QMainWindow (BTCturquoiseGUI)
...
#include <qt/btcturquoise.moc>

BTCturquoiseCore::BTCturquoiseCore(interfaces::Node& node) :
    QObject(), m_node(node)
{
}

void BTCturquoiseCore::handleRunawayException(const std::exception_ptr e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void BTCturquoiseCore::initialize()
{
    try
    {
...
void BTCturquoiseCore::restart(QStringList args)
{
...
void BTCturquoiseCore::shutdown()
{
    try
    {
...
BTCturquoiseApplication::BTCturquoiseApplication(interfaces::Node& node, int &argc, char **argv):
...
BTCturquoiseApplication::~BTCturquoiseApplication()
{
    if(coreThread)
    {
...
#ifdef ENABLE_WALLET
void BTCturquoiseApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

void BTCturquoiseApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(m_node, nullptr, resetSettings);
}

void BTCturquoiseApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BTCturquoiseGUI(m_node, networkStyle, 0);
    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, SIGNAL(timeout()), window, SLOT(detectShutdown()));
}

void BTCturquoiseApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void BTCturquoiseApplication::startThread()
{
    if(coreThread)
        return;
    coreThread = new QThread(this);
    BTCturquoiseCore *executor = new BTCturquoiseCore(m_node);
...
void BTCturquoiseApplication::parameterSetup()
{
...
void BTCturquoiseApplication::requestInitialize()
{
...
void BTCturquoiseApplication::requestShutdown()
{
...
void BTCturquoiseApplication::addWallet(WalletModel* walletModel)
{
#ifdef ENABLE_WALLET
...
void BTCturquoiseApplication::removeWallet()
{
#ifdef ENABLE_WALLET
...
void BTCturquoiseApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        qWarning() << "Platform customization:" << gArgs.GetArg("-uiplatform", BTCturquoiseGUI::DEFAULT_UIPLATFORM).c_str();
#ifdef ENABLE_WALLET
        ...
        // btcturquoise: URIs or payment requests:
...
void BTCturquoiseApplication::shutdownResult()
{
    quit(); // Exit second main loop invocation after shutdown finished
}

void BTCturquoiseApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception", BTCturquoiseGUI::tr("A fatal error occurred. BTCturquoise Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId BTCturquoiseApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BTCturquoiseGUI::DEFAULT_UIPLATFORM), true, OptionsCategory::GUI);
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"BTCturquoise Core - \""), false, OptionsCategory::GUI);
}

#ifndef BTCTURQUOISE_QT_TEST
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(btcturquoise);
    Q_INIT_RESOURCE(btcturquoise_locale);
    ...
    BTCturquoiseApplication app(*node, argc, argv);
    ...
    /// 6. Determine availability of data and blocks directory and parse btcturquoise.conf
    ...
    try {
        node->readConfigFile(gArgs.GetArg("-conf", BTCTURQUOISE_CONF_FILENAME));
    ...
    // btcturquoise: links repeatedly have their payment requests routed to this process:
