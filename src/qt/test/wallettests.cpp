...
#include <key_io.h>
#include <test/test_btcturquoise.h>
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount)
{
    ...
    entry->findChild<BTCturquoiseAmountField*>("payAmount")->setValue(amount);
    uint256 txid;
...
//
//     src/qt/test/test_btcturquoise-qt -platform xcb      # Linux
//     src/qt/test/test_btcturquoise-qt -platform windows  # Windows
//     src/qt/test/test_btcturquoise-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = BTCturquoiseUnits::floorHtmlWithUnit(unit, balance, false, BTCturquoiseUnits::separatorAlways);
    ...
    // Amount input
    BTCturquoiseAmountField* amountInput = receiveCoinsDialog.findChild<BTCturquoiseAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(2).indexOf(QString("URI: btcturquoise:")) != -1);
            QVERIFY(paymentTextList.at(3).indexOf(QString("Address:")) != -1);
            QCOMPARE(paymentTextList.at(4), QString("Amount: 0.00000001 ") + BTCturquoiseUnits::name(unit));
