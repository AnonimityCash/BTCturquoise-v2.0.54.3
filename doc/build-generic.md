GENERIC BUILD NOTES
====================
Some notes on how to build BTCturquoise Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build BTCturquoise with system-wide installed dependencies
...
------------------------------------
Building the dependencies and BTCturquoise Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
BTCturquoise inherited the `depends` folder from BTCturquoise, which contains all dependencies required to build BTCturquoise. These
dependencies must be built before BTCturquoise can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build BTCturquoise Core. Caching of build results will ensure that only
...
not done, build failures might occur when building BTCturquoise.
...
ccache
------
`./configure` of BTCturquoise Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
