...
*Setup instructions for a Gitian build of BTCturquoise Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BTCturquoise
...
- [Building BTCturquoise Core](#building-btcturquoise-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-btcturquoise-host/README.md).
...
Clone the git repositories for BTCturquoise Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/btcturquoiseverse/btcturquoise
...
in the BTCturquoise Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BTCturquoise Core
----------------

To build BTCturquoise Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the BTCturquoise Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/btcturquoise/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/btcturquoiseverse/btcturquoise
...
For example:
```bash
URL=https://github.com/crowning-/btcturquoise.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit btcturquoise=${COMMIT} --url btcturquoise=${URL} ../btcturquoise/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit btcturquoise=${COMMIT} --url btcturquoise=${URL} ../btcturquoise/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit btcturquoise=${COMMIT} --url btcturquoise=${URL} ../btcturquoise/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the btcturquoise git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../btcturquoise/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/btcturquoise
SIGPATH=/some/root/path/btcturquoise-detached-sigs

./bin/gbuild --url btcturquoise=${BTCPATH},signature=${SIGPATH} ../btcturquoise/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/btcturquoise-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/btcturquoise-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/btcturquoise-osx-build.assert
```
...
[btcturquoise/gitian.sigs](https://github.com/btcturquoiseverse/gitian.sigs/) repository, or if that's not possible to create a pull
request.
