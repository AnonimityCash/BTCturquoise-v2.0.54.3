...
- [Development guidelines](#development-guidelines)
    - [General BTCturquoise Core](#general-btcturquoise-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BTCturquoise Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/btcturquoiseverse/btcturquoise/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/btcturquoised -printtoconsole
```
...
BTCturquoise Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as BTCturquoise Core, where everyone uses
...
pay attention to for reviewers of BTCturquoise Core code.

General BTCturquoise Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BTCturquoise Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `BTCTURQUOISE_FOO_BAR_H`, e.g.

```c++
#ifndef BTCTURQUOISE_FOO_BAR_H
#define BTCTURQUOISE_FOO_BAR_H
...
#endif // BTCTURQUOISE_FOO_BAR_H
```
...
Some of these are maintained by active developers of BTCturquoise Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against BTCturquoise Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/btcturquoise-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/btcturquoise-core/ctaes ; actively maintained by Core contributors.
...
file descriptors it will cause problems with BTCturquoise's `select()` loop, because
...
compatibility between nodes. This happened in BTCturquoise 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BTCturquoise's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:btcturquoise/btcturquoise.git
...
Commit [`bb81e173`](https://github.com/btcturquoise/btcturquoise/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `btcturquoise-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `btcturquoise-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BTCturquoise Core, the wallet was always
