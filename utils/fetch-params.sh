...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/BTCturquoiseParams"
else
    PARAMS_DIR="$HOME/.btcturquoise-params"
fi
...
DOWNLOAD_URL="https://github.com/btcturquoiseverse/btcturquoise/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with BTCQ_DISABLE_SOMETHING=1
BTCQ_DISABLE_WGET="${BTCQ_DISABLE_WGET:-}"
#BTCQ_DISABLE_IPFS="${BTCQ_DISABLE_IPFS:-}"
BTCQ_DISABLE_CURL="${BTCQ_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$BTCQ_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$BTCQ_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$BTCQ_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the BTCturquoise parameters!
...
main() {
    ...
    cat <<EOF
BTCQ - fetch-params.sh

This script will fetch the BTCturquoise SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BTCturquoise SNARK parameters. Note that it is
