...
# btcturquoise_hash
RUN git clone https://github.com/btcturquoiseverse/btcturquoise_hash
RUN cd btcturquoise_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} btcturquoise
RUN useradd -u ${USER_ID} -g btcturquoise -s /bin/bash -m -d /btcturquoise btcturquoise
...
RUN mkdir /btcturquoise-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /btcturquoise-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /btcturquoise-src
