...
export BUILDER_IMAGE_NAME="btcturquoise-builder-$BUILD_TARGET-$JOB_NUMBER"
...
export GOAL="install"
export SDK_URL=${SDK_URL:-https://btcturquoisecore.org/depends-sources/sdks}
export MAKEJOBS="-j4"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export BTCTURQUOISE_CONFIG="--enable-glibc-back-compat --enable-reduce-exports CXXFLAGS=-Wno-psabi"
elif [ "$BUILD_TARGET" = "win32" ]; then
  export HOST=i686-w64-mingw32
  export DPKG_ADD_ARCH="i386"
  export BTCTURQUOISE_CONFIG="--enable-gui --enable-reduce-exports --disable-miner"
  ...
  export BTCTURQUOISE_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-crash-hooks LDFLAGS=-static-libstdc++"
  export USE_SHELL="/bin/btcturquoise"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  ...
  export BTCTURQUOISE_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-crash-hooks"
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_BTCturquoise_DEBUG"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  export RUN_INTEGRATIONTESTS=true
  ...
  export BTCTURQUOISE_CONFIG="--enable-glibc-back-compat --enable-reduce-exports"
  export RUN_UNITTESTS=true
elif [ "$BUILD_TARGET" = "linux64_release" ]; then
  export HOST=x86_64-unknown-linux-gnu
  export DEP_OPTS="NO_UPNP=1"
  export BTCTURQUOISE_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports"
  export PYZMQ=true
  export RUN_UNITTESTS=true
